    ...
    mouseEnter(event) {
        ...
        for (let i = 0; i < seekBar.length; i++) {
            if (seekBar[i] instanceof HTMLElement) {
                ...
                const sppb = mlv + mbvs;
                const tmv = this.options.thumbnails.filter(tm => (event.clientX - sppb - sb.offsetWidth * (tm.endTime ? tm.endTime : this.video.duration) / this.video.duration) < 0)[0];
                const tmi = this.options.thumbnails.findIndex(tm => (event.clientX - sppb - sb.offsetWidth * (tm.endTime ? tm.endTime : this.video.duration) / this.video.duration) < 0);
                if (tmv && tmi !== null) {
                    const thumbnailPixel = sb.offsetWidth / (this.video.duration / tmv.displayTime);
